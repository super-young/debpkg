...
static bstring DB_load(){
    ...
    check(data, "failed to read data from db");
...
int DB_update(const char * url){
    ...
    int rc = fwrite(line,blength(line),1,db);
    check(rc==1,"failed to write url into database");
...
static int DB_find(const char * url){
        bstring data = NULL;
        bstring line = bfromcstr(url);
        ...
        data = DB_load();
        check(data,"failed to load data from database");
